...
const LandOfficerDashboard = () => {
  ...
  // in here at the first load loading data using fetch then from the socket events came according to event data will manaipilated if event is created then want to addedt to exstotaing data array id updated then want to find the data and update it
  useWebSocket("8065", userSessionId, (content) => {
    console.log(content);
    setstatsFromRes(content.stats)
    console.log(statsFromRes)
  });
  ...
  const [verificationItems, setVerificationItems] = useState([
    ...
    },
    {
      ...
      verificationNotes:
        "ඔප්පුව සත්‍යාපනය කර ඇත, හැඳුනුම්පත තවමත් පරීක්ෂා කරමින් පවතී",
    },
  ]);
    ...
    setDisputes((prev) =>
      prev.map((dispute) =>
        dispute.id === disputeId
          ? { ...dispute, assignedOfficer: officerId }
          : dispute
      )
    );
  };
